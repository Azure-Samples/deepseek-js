...
export class ChatComponent extends LitElement {
      ...
      <div class="message ${message.role} animation">
        ...
        <div class="message-body">
          ${message.role === 'assistant'
            ? html`<details class="thoughts">
                <summary>
                  <span class="think-icon">${unsafeSVG(lightbulbSvg)}</span>
                  ${this.options.strings.thoughtsTitle}
                  <span class="${message.hasContent ? '' : 'dot-animation'}"></span>
                </summary>
                <div class="content">${message.thoughts}</div>
              </details>`
            : nothing}
  ...
  static override styles = css`
    :host {
      ...
      --highlight-bg: var(--azc-highlight-bg, #f2f2f2);
    ...
    *:focus-visible {
    ...
    .dot-animation:before {
      ...
      inset: 0;
      margin: auto;
      animation: moveX 1s cubic-bezier(0.5, 100, 0.5, -100) infinite;
    }
    @keyframes ballX {
      0%,
      25%,
